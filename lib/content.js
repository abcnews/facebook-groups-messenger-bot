    ...
    request.get(
      ...
      function(err, response, body) {
        ...
        $('item').each(function(){
          ...
          const teaserTitle = striptags($(this).find('title').text().trim());
          const shortTeaserTitle = teaserTitle;
          const shortTeaserTextPlain = striptags($(this).find('description').text().trim());
          const imageUrl = $(this).find('[medium="image"]').eq(0).attr('url');
          const thumbnailLink = {
            id: imageUrl ? toCmid(imageUrl) : null,
          };
          const item = { canonicalUrl, shortTeaserTitle, shortTeaserTextPlain, imageUrl, teaserTitle, thumbnailLink };
